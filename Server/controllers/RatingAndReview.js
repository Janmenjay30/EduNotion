...
exports.createRating=async(req,res)=>{
    try{
        ...
        const ratingReview=await RatingAndReview.create({
            course:courseId,user:userId,rating,review,
        }
        ) 
        // update course and rating and review 
        const updatedCourseDeails= await Course.findByIdAndUpdate({_id:courseId},
            {
                $push:{
        ...
        console.log(updatedCourseDeails);
        // return response 
...
exports.getAllRating = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            data: allReviews, // Return fetched reviews
        });
    } catch (error) {
        console.error(error);
        return res.status(500).json({
